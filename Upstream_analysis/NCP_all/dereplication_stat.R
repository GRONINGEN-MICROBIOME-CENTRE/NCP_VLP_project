##########################################
# Parsing VC data and VC size after
# dereplication
##########################################
args = commandArgs(trailingOnly=TRUE)
##############################
# Loading libraries
##############################
.libPaths("/home1/p309176/R_libraries")
library(tidyverse)
##############################
# Functions
##############################

##############################
# Input data
##############################
viral_clusters <- read_delim(args[1], col_names = F)
##############################
# ANALYSIS
##############################

# Transforming into long format to merge with Extended_TOF
long_df <- viral_clusters %>%
  separate_rows(X2, sep = ",\\s*") %>%
  arrange(X2, X1) %>%
  rename(Representative = X1, Cluster_member = X2)

# Getting the size of dereplication cluster
cluster_counts <- long_df %>%
  count(Representative) %>%
  rename(Cluster_size = n)

write.table(long_df, paste0(sub('\\.tsv', '', args[1]), '_long_format.txt'), sep='\t', row.names = F, quote=F)
write.table(cluster_counts, paste0(sub('\\.tsv', '', args[1]), '_size.txt'), sep='\t', row.names = F, quote=F)

MERGED_Extended_TOF_NCP <- read.delim("./MERGED_Extended_TOF_NCP")

merged_long_counts <- merge(long_df, cluster_counts, by="Representative")
MERGED_Extended_TOF_NCP$Representative <- MERGED_Extended_TOF_NCP$New_CID
MERGED_Extended_TOF_NCP_new <- merge(MERGED_Extended_TOF_NCP, merged_long_counts, by="Representative", all.y = T)
MERGED_Extended_TOF_NCP_new$Representative <- NULL

lines_vector <- readLines("./virus_representatives_NCs_filtered.txt")
lines_vector <- trimws(lines_vector)
lines_vector <- gsub("[[:cntrl:]]", "", lines_vector)

MERGED_Extended_TOF_NCP_new <- MERGED_Extended_TOF_NCP_new %>%
	mutate(representative_present_in_NC = ifelse(New_CID %in% lines_vector, "Yes", "No"))

write.table(MERGED_Extended_TOF_NCP_new, "MERGED_Extended_TOF_NCP_clean", sep='\t', row.names = F, quote=F)
